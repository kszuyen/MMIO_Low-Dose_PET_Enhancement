...
if __name__ == "__main__":
    ...
    with open(os.path.join(DIR_PATH, json_file) , 'r') as openfile:
                        ...
                        for i in range(mini_batch_size):
                            ...
                            if not CALCULATED: # only need to calculate once for each patient
                                ...
                                if orig_psnr <= psnr_threshold:
                                    ...
                                    remove_slice.add((patient, batch_slc[i]))

                            """  new score  """
                            if (patient, batch_slc[i]) not in remove_slice:
        ...
        for k, l in enumerate(all_score):
            if l and k == 0:
                l += ["mean", "std"]
            elif l and k != 0:
                l += [mean(l), std(l)]
